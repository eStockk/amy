...
- `22/tcp` SSH
- `80/tcp` HTTP for Nginx/Let's Encrypt
...
The public reverse proxy is Nginx. Its config template lives at:

```text
/opt/amy/app/monitoring/nginx/nginx.conf.template
```

Certificates are managed by the `certbot` container and stored in the `letsencrypt-data` Docker volume.

After changing Nginx config:

```bash
cd /opt/amy/app
docker-compose -p app -f docker-compose.yml up -d --build nginx certbot
